...
class RobustDDPGAgent(BaseAgent):
    def __init__(self, config):
        ...
        if attack_config['enabled']:
            ...
            self.attack_rate = attack_config['rate']
        ...
        if self.OARL:
            if os.path.exists('OARL_prediction/{}'.format(self.config.OARL_params['model_name'])):
                if self.config.OARL_params['model_name'].endswith('keras'):
                    self.oarl_model = keras.models.load_model(
                        'OARL_prediction/{}'.format(self.config.OARL_params['model_name']))
    ...
    def attack_state(self, state, step=None, lstm=False):
        if step is not None:
            if lstm and step > 3:
    ...
    def set_epsilon(self, eps):
        self.attack_epsilon = eps

    def get_epsilon(self):
        return self.attack_epsilon
    ...
    def eval_step(self, state, certify_eps=0.0, try_attack=True, step=None, lstm=False):
        ...
        attacked = 0
        if self.enabled_attack and try_attack:
            if np.random.random() < self.attack_rate:
                state, attacked = self.attack_state(state, step, lstm)
            else:
                attacked = 0
        ...
        if certify_eps > 0.0:
            ...
            return to_np(action), actor_l1, actor_l2, actor_linf, actor_diff, attacked, gt_state, state
        self.config.state_normalizer.unset_read_only()
        return to_np(action), attacked, gt_state, state
